...
import { isIndexAsset, isCryptoAsset } from './config/assets'
...
export default function App() {
  ...
  const [mobileStockOpen, setMobileStockOpen] = useState(true)

  const asset = assets.find(a => a.id === selectedAsset)
  const isLoggedIn = !!auth.user
  const isIndex = isIndexAsset(asset?.id)
  const isCrypto = isCryptoAsset(asset?.id)
  const isStock = !isIndex && !isCrypto && !!asset?.yahooSymbol
        ...
        <div className="w-[35%] bg-bg-panel border-l border-border flex flex-col overflow-hidden">
          ...
          <div className="flex border-b border-border shrink-0">
            {['markets', 'news', ...(isIndex ? ['economy'] : []), ...(isStock ? ['stock'] : [])].map(tab => (
              ...
              >
                {tab === 'markets' ? 'Event Markets' : tab === 'news' ? 'News' : tab === 'economy' ? 'Economy' : 'Stock'}
              </button>
            ))}
          </div>
      ...
      <div className="md:hidden flex flex-col h-dvh overflow-hidden bg-bg-primary">
        ...
        <div className="flex-1 overflow-y-auto border-t border-border">
          <div className="bg-bg-panel">
            ...
            {/* Collapsible Economy section (indices only) */}
            {isIndex && (
              <>
                <button
                  onClick={() => setMobileEconOpen(prev => !prev)}
                  className="w-full flex items-center justify-between px-3 py-2.5 border-b border-border"
                >
                  <span className="text-xs font-semibold uppercase tracking-wider text-text-secondary">
                    Economy
                  </span>
                  <svg className={`w-3.5 h-3.5 text-text-secondary transition-transform ${mobileEconOpen ? 'rotate-180' : ''}`}
                    viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <polyline points="6 9 12 15 18 9"/>
                  </svg>
                </button>
                <div className={`p-3 ${!mobileEconOpen ? 'hidden' : ''}`}>
                  <EconPanel enabled={true} />
                </div>
              </>
            )}

            {/* Collapsible Stock Summary section (stocks only) */}
            {isStock && (
              <>
                <button
                  onClick={() => setMobileStockOpen(prev => !prev)}
                  className="w-full flex items-center justify-between px-3 py-2.5 border-b border-border"
                >
                  <span className="text-xs font-semibold uppercase tracking-wider text-text-secondary">
                    Stock Summary
                  </span>
                  <svg className={`w-3.5 h-3.5 text-text-secondary transition-transform ${mobileStockOpen ? 'rotate-180' : ''}`}
                    viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <polyline points="6 9 12 15 18 9"/>
                  </svg>
                </button>
                <div className={`p-3 ${!mobileStockOpen ? 'hidden' : ''}`}>
                  <StockPanel asset={asset} enabled={true} />
                </div>
              </>
            )}
          </div>
